...
function replaceText(word, replacement, text) {
  let a = new RegExp(word, "g");
  let b = text.replace(a, replacement);
  return b;
...
function countEmails(str) {
  const e = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/g;
  const m = str.match(e);
...
function checkRegexFlags(regex) {
  ...
  const a = flags.includes("g");
  const b = flags.includes("m");
  return a && b;
